...
"""
        ...
        # Step 5: Run the vina command for each config file
        vina_command = f"vina --config {config_filename}"
        print(f"Running: {vina_command}")
        
        # Execute the vina command
        try:
            result = subprocess.run(vina_command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            print(result.stdout.decode())
        except subprocess.CalledProcessError as e:
            print(f"Error occurred while running {vina_command}:")
            print(e.stderr.decode())

print("All Vina processes have been executed.")
