### 도킹 데이터로부터 molecular descriptor를 계산하고, 이를 docking score와 함께 결합하여 최종 데이터를 저장
#!/usr/bin/env python
# coding: utf-8
...
df_raw = pd.read_csv("merged_300K.csv") #csv file load
...
# 'SMILES' 열에 있는 각 SMILES 문자열에 대해 MACCS 키를 생성하고,
# 이를 'MACCS'라는 새로운 열에 추가합니다.
df['MACCS'] = df['SMILES'].apply(generate_MACCS)
...
radius = 2; nbits = 2048 # radius=1 ECFP2 / radius=2 ECFP4
fp_list = []
for m in tqdm(mols):
...
##RDKit descriptor 생성import numpy as np
...
for m in tqdm(mols):
  ...
  rDKdesc_list.append(desc)


# In[ ]:


desc_df = pd.DataFrame(rDKdesc_list)
object_dtype = [ idx for idx, i in enumerate(desc_df.dtypes) if i == 'object' ]
rdk_desc_df = desc_df.drop(desc_df.columns[object_dtype], axis=1)
...
###dataframe 합치기
merged_df = pd.concat([score_df, fp_df, rdk_desc_df, maccs_df], axis=1)
merged_df.to_csv('merged300K_desc.csv', index=False)
